/**
    ...
    File IO operations to disk

*/
namespace file
{
    internal void free_binary(BinaryFileHandle& binary_file_to_free)
    {
    ...
    internal void read_binary(BinaryFileHandle& mem_to_read_to, const char* file_path)
    {
        if(mem_to_read_to.memory)
        {
            console::cprintf("WARNING: Binary File Handle already points to allocated memory. Freeing memory first...\n");
            file::free_binary(mem_to_read_to);
        }
        ...
        {
            console::cprintf("Failed to read %s! File doesn't exist.\n", file_path);
            return;
        }
    }

    /** Returns the string content of a file as an std::string */
    internal std::string read_string(const char* file_path)
    {
    ...
    internal void free_image(BitmapHandle& image_handle)
    {
        file::free_binary(image_handle);
    ...
    /** Allocates memory, loads an image file as an UNSIGNED BYTE bitmap, makes BitmapHandle.memory
        ...
        bitmap information. */
    internal void read_image(BitmapHandle& image_handle, const char* image_file_path)
    {
        if(image_handle.memory)
        {
            console::cprintf("WARNING: Binary File Handle already points to allocated memory. Freeing memory first...\n");
            file::free_image(image_handle);
        }
        ...
        {
            console::cprintf("Failed to find image file at: %s\n", image_file_path);
            image_handle.width = 0;
            image_handle.height = 0;
            image_handle.bit_depth = 0;
            return;
