namespace debug
{
    ...
    internal Mesh create_circle_mesh(real32 pos_x, real32 pos_y, real32 pos_z, real32 radius, uint8 axis=0)
    {
        ...
        return gl::create_mesh_array(vertices, indices, vertex_count * 3, indices_count, 3, 0, 0);
    }
    ...
    internal void render_sphere(PerspectiveShader& shader, real32 x, real32 y, real32 z, real32 radius)
    {
        ...
        gl::bind_model_matrix(shader, sphere_transform.ptr());
        gl::render_mesh(debug_sphere_mesh, GL_LINES);
    ...
    internal void render_cone(PerspectiveShader& shader,
                                    real32 x, real32 y, real32 z,
                                    real32 height, real32 base_radius,
                                    quaternion orientation)
    {
        ...
        gl::bind_model_matrix(shader, cone_transform.ptr());
        gl::render_mesh(debug_cone_mesh, GL_LINES);
    }

    internal void render_line()
    {
        // TODO
    }

    internal real32 calculate_attenuation_range(real32 c, real32 l, real32 q)
    {
    ...
    internal void render_pointlight(PerspectiveShader& shader, PointLight& plight)
    {
        real32 att_radius = debug::calculate_attenuation_range(plight.att_constant,
                                                              plight.att_linear,
        ...
        if(id_uni_frag_colour != 0xffffffff)
        {
            ...
            debug::render_sphere(shader, plight.position.x, plight.position.y, plight.position.z, att_radius);
            glUniform4f(id_uni_frag_colour, 1.f, 1.f, 0.f, 1.f);
            debug::render_sphere(shader, plight.position.x, plight.position.y, plight.position.z, 0.05f);
        }
    }

    internal void render_spotlight(PerspectiveShader& shader, SpotLight& slight)
    {
        real32 att_radius = debug::calculate_attenuation_range(slight.att_constant,
                                                          slight.att_linear,
        ...
        if(id_uni_frag_colour != 0xffffffff)
        {
            ...
            debug::render_cone(shader, slight.position.x, slight.position.y, slight.position.z,
                height, base_radius, slight.orientation);
            glUniform4f(id_uni_frag_colour, 1.f, 1.f, 0.f, 1.f);
            debug::render_sphere(shader, slight.position.x, slight.position.y, slight.position.z, 0.05f);
        }
    }
    ...
    internal void render(PerspectiveShader& debug_shader)
    {
        if(!debugger_level)
        {
            return;
        ...
        gl::use_shader(debug_shader);
            gl::bind_view_matrix(debug_shader, g_camera.matrix_view.ptr());
            gl::bind_projection_matrix(debug_shader, g_camera.matrix_perspective.ptr());
            ...
            if(1 <= debugger_level)
            {
                if(debugger_b_debug_pointlights)
                {
                    ...
                    {
                        debug::render_pointlight(debug_shader, debugger_point_lights[i]);
                    }
                }
                ...
                if(debugger_b_debug_spotlights)
                {
                    ...
                    {
                        debug::render_spotlight(debug_shader, debugger_spot_lights[i]);
                    }
                }
    ...
    internal void set_debug_pointlights(PointLight* point_lights_array, uint8 count)
    {
        debugger_point_lights = point_lights_array;
        debugger_point_lights_count = count;
    }

    internal void toggle_debug_pointlights()
    {
        debugger_b_debug_pointlights = !debugger_b_debug_pointlights;
    }

    internal void set_debug_spotlights(SpotLight* spot_lights_array, uint8 count)
    {
        debugger_spot_lights = spot_lights_array;
        debugger_spot_lights_count = count;
    }

    internal void toggle_debug_spotlights()
    {
