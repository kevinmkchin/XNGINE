...
namespace gl
{
    ...
    */
    internal void compile_shader(uint32 program_id, const char* shader_code, GLenum shader_type)
    {
        ...
        if (!result)
        {
            ...
            console::cprintf("Error compiling the %d shader: '%s' \n", shader_type, eLog);
            return;
        }
        // Attach to program
    ...
    internal void create_shader_program(BaseShader& shader, const char* vertex_shader_str, const char* fragment_shader_str)
    {
        // Create an empty shader program and get the id
        shader.id_shader_program = glCreateProgram();
        if (!shader.id_shader_program)
        {
            console::cprintf("Failed to create shader program! Aborting.\n");
            return;
        }
        // Compile and attach the shaders
        gl::compile_shader(shader.id_shader_program, vertex_shader_str, GL_VERTEX_SHADER);
        gl::compile_shader(shader.id_shader_program, fragment_shader_str, GL_FRAGMENT_SHADER);
        ...
        if (!result)
        {
            ...
            console::cprintf("Error linking program: '%s'! Aborting.\n", eLog);
            return;
        }
        ...
        {
            glGetProgramInfoLog(shader.id_shader_program, sizeof(eLog), nullptr, eLog);
            console::cprintf("Error validating program: '%s'! Aborting.\n", eLog);
            return;
        }
    ...
    internal void load_shader_program_from_file(BaseShader& shader, const char* vertex_path, const char* fragment_path)
    {
        std::string v = file::read_string(vertex_path);
        std::string f = file::read_string(fragment_path);
        gl::create_shader_program(shader, v.c_str(), f.c_str());
    }

    /** Telling opengl to start using this shader program */
    internal void use_shader(BaseShader& shader)
    {
        if (shader.id_shader_program == 0)
        {
            console::cprintf("WARNING: Passed an unloaded shader program to gl::use_shader! Aborting.\n");
            return;
        }
    ...
    internal void delete_shader(BaseShader& shader)
    {
        if (shader.id_shader_program == 0)
        {
            console::cprintf("WARNING: Passed an unloaded shader program to gl::delete_shader! Aboring.\n");
            return;
        }
        glDeleteProgram(shader.id_shader_program);
    }

    internal void bind_model_matrix(BaseShader& shader, const GLfloat* matrix)
    {
        glUniformMatrix4fv(shader.id_uniform_model, 1, GL_FALSE, matrix);
    }

    internal void bind_view_matrix(PerspectiveShader& shader, const GLfloat* matrix)
    {
        glUniformMatrix4fv(shader.id_uniform_view, 1, GL_FALSE, matrix);
    }

    internal void bind_projection_matrix(PerspectiveShader& shader, const GLfloat* matrix)
    {
        glUniformMatrix4fv(shader.id_uniform_proj_perspective, 1, GL_FALSE, matrix);
    }

    internal void bind_projection_matrix(OrthographicShader& shader, const GLfloat* matrix)
    {
        glUniformMatrix4fv(shader.id_uniform_proj_orthographic, 1, GL_FALSE, matrix);
    }

    internal void bind_camera_position(LightingShader& shader, Camera& camera)
    {
        glUniform3f(shader.id_uniform_observer_pos, camera.position.x, camera.position.y, camera.position.z);
    }

    internal void bind_directional_light(LightingShader& shader, DirectionalLight& light)
    {
    ...
    internal void bind_point_lights(LightingShader& shader, PointLight* plights, uint8 count)
    {
    ...
    internal void bind_spot_lights(LightingShader& shader, SpotLight* slights, uint8 count)
    {
    ...
    internal void bind_material(LightingShader& shader, Material& material)
    {
    ...
    */
    ...
    internal Mesh create_mesh_array(real32* vertices, 
                                       uint32* indices,
    ...
    internal void rebind_buffers(Mesh& mesh,
                                    real32* vertices, 
                                    uint32* indices,
                                    uint32 vertices_array_count,
                                    uint32 indices_array_count,
    ...
    /** Binds VAO and draws elements. Bind a shader program and texture before calling gl::render_mesh */
    internal void render_mesh(Mesh& mesh, GLenum mode = GL_TRIANGLES)
    {
        if (mesh.index_count == 0) // Early out if index_count == 0, nothing to draw
        {
            console::cprintf("WARNING: Attempting to render a mesh with 0 index count!\n");
            return;
        }
    ...
    internal void delete_mesh(Mesh& mesh)
    {
        if (mesh.id_ibo != 0)
        {
    ...
    */
    ...
    internal void delete_texture(Texture& texture)
    {
        if(texture.texture_id == 0)
        {
            console::cprintf("WARNING: Attempting to clear a texture with id: 0. This means this texture hasn't been loaded!\n");
            return;
        }
    ...
    internal void load_texture_from_bitmap(Texture&          texture,
                                              ...
                                              GLenum            source_format)
    {
        if(texture.texture_id != 0)
        {
            console::cprintf("WARNING: Trying to load a Texture when there is already a texture loaded! Clearing texture first...\n");
    ...
    internal void load_texture_from_file(Texture&    texture,
                                            const char* texture_file_path)
    {
        BitmapHandle texture_handle;
        file::read_image(texture_handle, texture_file_path);
            gl::load_texture_from_bitmap(texture, (unsigned char*)texture_handle.memory, texture_handle.width, 
                texture_handle.height, GL_RGBA, (texture_handle.bit_depth == 3 ? GL_RGB : GL_RGBA));
        file::free_image(texture_handle); // texture data has been copied to GPU memory, so we can free image from memory
    }
    ...
    internal void use_texture(Texture& texture)
    {
